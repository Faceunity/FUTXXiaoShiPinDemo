...
- (void)layoutSubviews {
    ...
    if (![MaterialManager isOnlinePackage:_materialID]) {
        [self.icon sd_setImageWithURL:nil];
        ...
        if ([[SDWebImageManager sharedManager].imageCache diskImageDataExistsWithKey:[[SDWebImageManager sharedManager] cacheKeyForURL:iconURL]]) {
            [self startAnimation:NO];
        } else {
