...
- (void)iniSubViews
{
    ...
    self.leftPin = ({
        ...
        imageView.height = _appearanceConfig.thumbHeight + 2 * _appearanceConfig.borderHeight;
...
- (void)unpdateBorder
{
    ...
    self.topBorder.width = self.rightPinCenterX - self.leftPinCenterX - _appearanceConfig.pinWidth;
    self.topBorder.y = 0;
    self.topBorder.x = self.leftPinCenterX + _appearanceConfig.pinWidth / 2;
    
    self.bottomBorder.height = _appearanceConfig.borderHeight;
    self.bottomBorder.width = self.rightPinCenterX - self.leftPinCenterX - _appearanceConfig.pinWidth;
    self.bottomBorder.y = self.leftPin.bottom-_appearanceConfig.borderHeight;
    self.bottomBorder.x = self.leftPinCenterX + _appearanceConfig.pinWidth / 2;
}
...
- (void)handleCenterPan:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan || gesture.state == UIGestureRecognizerStateChanged || gesture.state == UIGestureRecognizerStateEnded) {
        ...
        if (_centerPinCenterX > self.width - _appearanceConfig.pinWidth) {
            _centerPinCenterX = self.width - _appearanceConfig.pinWidth;
        }
