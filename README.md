...
由于腾讯个别库的大小过大，无法上传github，需要下载腾讯[Demo](http://download-1252463788.file.myqcloud.com/xiaoshipin/TXXiaoShiPinDemo_ios.zip?_ga=1.112422663.284137496.1526352913)并将demo中的Framework文件夹拷贝到我门对接的项目中。
...
1、在  TCVideoRecordViewController.m  中添加头文件，并创建页面属性
...
推流开始，在 TCVideoRecordViewController.m  的  `viewDidLoad： `  遵循腾讯SDK数据回调代理 `TXVideoCustomProcessDelegate`   ，并且实现代理方法 `-(GLuint)onPreProcessTexture:(GLuint)texture width:(CGFloat)width height:(CGFloat)height `  如下
